...
public class EcoAndPermissionHelper {
    ...
    static private PlayerPointsAPI playerPointsAPI;
    ...
    public static void setup() {
        ...
        if (getServer().getPluginManager().getPlugin("PlayerPoints") != null) {
            PlayerPoints playerPointsPlugin = (PlayerPoints) getServer().getPluginManager().getPlugin("PlayerPoints");
            playerPointsAPI = playerPointsPlugin.getAPI();
            log(Level.INFO, "找到PlayerPoints插件，已启用点券支持！");
        } else {
            log(Level.INFO, "未找到PlayerPoints插件，点券功能将不可用。");
        }
    }
    ...
    public static boolean givePoints(OfflinePlayer player, int amount) {
        return playerPointsAPI != null && playerPointsAPI.give(player.getUniqueId(), amount);
    }

    public static boolean takePoints(OfflinePlayer player, int amount) {
        return playerPointsAPI != null && playerPointsAPI.take(player.getUniqueId(), amount);
    }

    public static boolean hasPermission(CommandSender sender, String perNode) {
        boolean hasPerNode = per == null || !(sender instanceof Player) ? sender.hasPermission(perNode) : per.playerHas((Player)sender, perNode);
        return checkAndDoSomething(hasPerNode, emptyFunction, () -> sendSimpleMsg(sender, ChatColor.RED, "你没有进行此操作的权限！"));
    }

    public static boolean hasPermissionSilently(CommandSender sender, String perNode) {
        return per == null || !(sender instanceof Player) ? sender.hasPermission(perNode) : per.playerHas((Player)sender, perNode);
    }
