...
public class CommandHandler implements TabExecutor {
                    ...
                    if (checkArgs(args, 3, player) && getSessionManager().hasSession(player)) {
                        switch (args[1]) {
                            ...
                            case "currencytype":
                                switch (args[2].toLowerCase()) {
                                    case "money":
                                        getSessionManager().getSession(player).getBuilder().currencyType(RedPacket.CurrencyType.MONEY);
                                        break;
                                    case "points":
                                        getSessionManager().getSession(player).getBuilder().currencyType(RedPacket.CurrencyType.POINTS);
                                }
                        }
                    ...
                    if (checkArgs(args, 2, player)) {
                        ...
                        switch (args[1].toLowerCase()) {
                            ...
                            case "givetype":
                                ...
                                break;
                            case "currencytype":
                                if (checkSessionAndSetState(player, CreateSession.State.WaitCurrencyType)) {
                                    sendSimpleMsg(player, ChatColor.GREEN, "请选择货币类型：");
                                    sendSimpleMsg(player,
                                            new ComponentBuilder(ChatColor.GREEN + ChatColor.UNDERLINE.toString() + "金钱").event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/redpacket set currencytype money")).event(selectTip)
                                                    .append("  ").reset()
                                                    .append(ChatColor.GREEN + ChatColor.UNDERLINE.toString() + "点券").event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/redpacket set currencytype points")).event(selectTip)
                                                    .create());
                                }
                                break;
                            case "money":
                            ...
                            case "create":
                                Bukkit.getScheduler().runTaskAsynchronously(getInstance(), () -> {
                                    if (getSessionManager().getSession(player).getBuilder().isValid()) {
                                        ...
                                        //生成提示信息
                                        String extraDataFormatted = ColorHelper.format(redPacket.getExtraData());
                                        String text = ChatColor.GREEN + "玩家" + ChatColor.GOLD + player.getName() + ChatColor.GREEN + "发了一个" + (redPacket.isLimitPlayer() ? "只限" + redPacket.getLimitPlayerList() + "领取的" : "所有人的") + ChatColor.RESET + ChatColor.GREEN + redPacket.getType().getName() + "！ (" + redPacket.getType().getExtraDataName() + "：" + extraDataFormatted + ChatColor.GREEN + ")  ";
                                        //不含领取的提示信息
                                        ...
                                        switch (redPacket.getType()) {
                                            ...
                                            case PasswordRedPacket:
                                                String cleanPassword = ChatColor.stripColor(redPacket.getExtraData()).trim();
                                                componentBuilder.append(ChatColor.GREEN + ChatColor.UNDERLINE.toString() + "点击这里填充口令").underlined(true).event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, cleanPassword));
                                                break;
                                            case JieLongRedPacket:
                                                componentBuilder.append(ChatColor.GREEN + "下一个成语的音节为 " + ChatColor.GOLD + ChatColor.UNDERLINE.toString() + IdiomManager.getIdiomPinyin(redPacket.getExtraData()));
                                        }
                                        //对专享红包进行判断
                                        //防止游戏体验降低
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ArrayList<String> firstLevel = new ArrayList<>(Arrays.asList("new", "add", "help"));
            if (hasPermission(sender, "redpacket.command.reload")) {
                firstLevel.add("reload");
            }
            if (sender.isOp()) {
                firstLevel.addAll(Arrays.asList("query", "session", "set"));
            }
            ...
            switch (args[0].toLowerCase()) {
                case "query":
                    List<String> queryOptions = Arrays.asList("type", "givetype", "currencytype", "money", "amount", "giver", "extradata");
                    for (String s : queryOptions) {
                        if (s.startsWith(input)) {
                            suggestions.add(s);
                        }
                    }
                    break;
                case "session":
                    List<String> sessionOptions = Arrays.asList("create", "cancel");
                    ...
                    break;
                case "set":
                    List<String> setOptions = Arrays.asList("type", "givetype", "currencytype");
            ...
            if (args[1].equalsIgnoreCase("type")) {
                List<String> types = Arrays.asList("normal", "password", "jielong");
                for (String s : types) {
                    if (s.startsWith(input)) {
                        suggestions.add(s);
                    }
                }
            } else if (args[1].equalsIgnoreCase("givetype")) {
                List<String> giveTypes = Arrays.asList("fixed", "luck");
                for (String s : giveTypes) {
                    if (s.startsWith(input)) {
                        suggestions.add(s);
                    }
                }
            } else if (args[1].equalsIgnoreCase("currencytype")) {
                List<String> currencyTypes = Arrays.asList("money", "points");
                for (String s : currencyTypes) {
                    if (s.startsWith(input)) {
                        suggestions.add(s);
                    }
                }
